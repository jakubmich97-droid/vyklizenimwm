...
const updateEstimate = () => {
  if (!spaceType || !spaceRange || !spaceValue || !estimateOutput) return;

  const rate = Number(spaceType.value);
  const meters = Number(spaceRange.value);
  spaceValue.textContent = String(meters);
  estimateOutput.textContent = `Orientačně: ${formatCzk(rate * meters)}`;
};
...
if (contactForm && formFeedback) {
  contactForm.addEventListener("submit", (event) => {
    ...
    formFeedback.textContent = "Děkujeme! Brzy se vám ozveme s návrhem řešení.";
    contactForm.reset();
    updateEstimate();
  });
}
